<script lang="ts">
  ...
  let currentTime = '';
  let currentDate = '';
  let attendanceStatus = 'not_checked_in'; // 'not_checked_in', 'checked_in', 'checked_out'
  ...
  $: attendanceInfo = getAttendanceStatusInfo();
</script>
    ...
    <CardContent>
      <div class="flex items-center gap-4">
        ...
        <div class="flex-1">
          ...
          <p class="text-gray-600">{user?.position?.name || 'Position'}</p>
    ...
    <CardContent>
      <div class="text-center mb-4">
        <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full {attendanceInfo.color}">
          <attendanceInfo.icon class="h-4 w-4" />
      ...
      {:else}
        <Button
          on:click={handleAttendanceAction}
